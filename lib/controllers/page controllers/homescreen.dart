...
class HomePageController extends GetxController {
  ...
  RxMap dataFromHive = {}.obs;

  // Rx variable to track if the data is fetched from the API
  RxBool dataFetched = false.obs;

 Future <void> loadData(BuildContext context) async {
    // Load data from Hive if available
    var data = homeDataBox.get('products');

    // If Hive data is available, pass it to the HomeScreen
    if (data != null) {
      dataFromHive.value = data;
      dataFetched.value = true;
    }
    // If Hive is empty, make the API call and update the UI
    await getHomeScreeData(context);
  }
